...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Function to toggle the visibility of the sliding div
  function toggleSlideInDiv() {
    rightSidebar.style.right = rightSidebar.style.right === '0px' ? '400px' : '0px';
  }

  // Event listener for image click to toggle sidebar visibility
  image2.addEventListener('click', toggleSlideInDiv);

  // Event listener to hide the sidebar when clicked outside
  document.addEventListener('click', function(event) {
    if (!rightSidebar.contains(event.target) && event.target !== image2) {
      rightSidebar.style.right = '-400px'; // Hide the sliding div
    }
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const items = [
    { category: starterItems, name: 'Soup', price: 5.99, imageSrc: './img/chef (2).png' },
    { category: starterItems, name: 'Salad', price: 7.99, imageSrc: './img/chef (2).png' },
    { category: meatItems, name: 'Steak', price: 15.99, imageSrc: './img/chef (2).png' },
    { category: meatItems, name: 'Burger', price: 9.99, imageSrc: './img/chef (2).png' },
    { category: fishItems, name: 'Salmon', price: 12.99, imageSrc: './img/chef (2).png' },
    { category: fishItems, name: 'Shrimp', price: 11.99, imageSrc: './img/chef (2).png' },
    { category: dessertItems, name: 'Cake', price: 8.99, imageSrc: './img/chef (2).png' },
    { category: dessertItems, name: 'Ice Cream', price: 6.99, imageSrc: './img/chef (2).png' }
  ];
  ...
  document.querySelectorAll('.plus-btn').forEach(button => {
    button.addEventListener('click', function() {
      const itemPrice = parseFloat(this.parentElement.nextElementSibling.textContent.slice(1));
